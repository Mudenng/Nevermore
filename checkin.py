    ...
    if len(today_records) == 0:
        ...
        if (hour * 60 + minute) <= (ondutyhour * 60 + ondutyminute):
            ...
            try:
                ...
                return ("您已完成考勤登记. 时间：%s . <br>考勤类型：%s . 考勤状态：%s ." % (now_str, "上班", "正常"), 1)
            except:
            ...
            try:
                db.add_checkin_record(sid = sid, rtype = rtype, rstate = rstate, rimage = image_path)
                return ("您已完成考勤登记. 时间：%s . <br>考勤类型：%s . 考勤状态：%s ." % (now_str, "上班", "有点晚"), 1)
            except:
                print "Error: Write checkin record to DB failed. sid = %s" % sid
                return ("好像发生了一些错误，请重试.", -1)
        ...
        if (hour * 60 + minute) <= (offdutyhour * 60 + offdutyminute):
            ...
            try:
                db.add_checkin_record(sid = sid, rtype = rtype, rstate = rstate, rimage = image_path)
                return ("您已完成考勤登记. 时间：%s . <br>考勤类型：%s . 考勤状态：%s ." % (now_str, "下班", "有点早"), 1)
            except:
                print "Error: Write checkin record to DB failed. sid = %s" % sid
                return ("好像发生了一些错误，请重试.", -1)
        else:
            ...
            try:
                db.add_checkin_record(sid = sid, rtype = rtype, rstate = rstate, rimage = image_path)
                return ("您已完成考勤登记. 时间：%s . <br>考勤类型：%s . 考勤状态：%s ." % (now_str, "下班", "正常"), 1)
            except:
                print "Error: Write checkin record to DB failed. sid = %s" % sid
                return ("好像发生了一些错误，请重试.", -1)
    else:
